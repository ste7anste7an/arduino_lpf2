...
// uncomment next loine to show full debug output
// #define DEBUG
    ...
    for (int i = modes - 1; i >= 0; i--) {
      ...
      /*
           mode_name.encode("ascii")
                      ...
                      INFO_NAME = 0x00
      */
      if (l <= 5) { // pad name with zero's until length 5
        mode->name.getBytes(&bb[1], l + 1); // Leave room for null terminator
        bb[7] = 0x80;
        bb[11] = 0x5;
        bb[12] = 0x4;
        ll = next_power2(13);
